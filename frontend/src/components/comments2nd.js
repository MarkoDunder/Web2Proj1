...
import commentsServiceR2 from '../services/commentsServiceR2';
...
function Comments2nd() {
  ...
  const addComment=()=>{
    /*CreateACommentService.create(comments).then(response =>{
      ...
      axios.post('http://localhost:6060/api/v1/comment',{
      comment_id:uuid(),
  ...
  const removeComment= async(comment_id, usersName)=>{
    ...
    /*let response= await deleteComment.remove(props).then(()=>{
      ...
      const response= await axios.delete(`http://localhost:6060/api/v1/deleteComment/${comment_id}`).then(
        ()=>{console.log("success");}
      )
  ...
  const editComment=async(text, usersName, comment_id)=>{
    if((usersName===(userName))|| user.name==adminName){
        const response = await axios.put(`http://localhost:6060/api/v1/updateComment/${comment_id}`, {
          comment_text: text
        }).then(
